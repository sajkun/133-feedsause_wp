...
// var stripe = Stripe("pk_test_epxLdX0oCEqtGEBIzs7MFtC4003SgnBL0z");
// var theme_stripe;
...
//       gallery_app.$refs.order_details.$refs.my_order_popup.card_inserted = false;
//       gallery_app.$refs.order_details.$refs.my_order_popup.show_blocker = false;
...
//         }, 4000);
//       } else {
//         gallery_app.$refs.order_details.$refs.my_order_popup.position = 'step_4';
...
Vue.component('select-imitation-type', {
  ...
  template: `
    <div class="select-imitation" v-click-outside="discard_select"  v-bind:class="{ expanded: isExpanded, error: error}" >
      ...
      <span class="select-imitation__view no-arrow" v-on:click="expand_select"  v-bind:class="{ hidden: isHiddenImitation}">{{selected}}</span>
      <span class="select-imitation__arrow " onclick="imitate_select_expand(this)">
        <svg class="icon svg-icon-bracket">
          <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-bracket"></use>
        </svg>
      </span>
      <div class="select-imitation__dropdown">
        <ul class="select-imitation__list">
          <li v-for="data in options" v-bind:class="{selected: isSelected[data], soon: !data.published}"  v-on:click="imitate_select_option(data.name, data.published)"> <span  class="element">{{data.name}} <span class="comming-soon" v-if="!data.published">COMING SOON</span></span> </li>
        </ul>
      </div>
    </div>`,
})
...
Vue.component('my-order-details',{
  ...
  computed: {
    ...
    item: function(){
      ...
      return {
        name: typeof( this.meta.item.item_name ) == 'object'? this.meta.item.item_name[0].split('-')[0] :  this.meta.item.item_name,
        count: count,
...
Vue.component('currency-swithcer',{
  ...
  computed:{
    currencies: function(){
      return {
        usd : {
          flag: WP_URLS.theme_url +'/images/svg/us.svg',
          currency_code: 'USD',
          index_gpb: currency_settings['usd'],
          symbol: '$',
        },
        gbp: {
          flag: WP_URLS.theme_url +'/images/svg/gb.svg',
          currency_code: 'GBP',
          index_gpb: 1,
          symbol: '£',
        },
        eur: {
          flag: WP_URLS.theme_url +'/images/svg/eu.svg',
          currency_code: 'EUR',
          index_gpb: currency_settings['eur'],
          symbol: '€',
        },
      };
    },
  ...
  methods:{
    set_currency: function(curr){
      this.currency = curr,
      this.dropdown = 0;
      this.$emit('change_currency', this.currencies[curr]);
    },
  ...
  template: `
    <div class="component-currency-switcher sm" :class="{'active': dropdown}" v-click-outside="discard_select">
      <div class="component-currency-switcher__value" v-on:click="dropdown=1">
        <img :src="currencies[currency].flag" alt="">
        {{currencies[currency].currency_code}}

        <div class="component-currency-switcher__value-arrow">
          <svg class="icon svg-icon-bracket">
            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#svg-icon-bracket"></use>
          </svg>
        </div>
      ...
      <div class="component-currency-switcher__dropdown">
        <ul>
          <li
          v-on:click="set_currency(id)"
           v-for="curr,id,key in currencies"
           :key="'curr'+id"
           :data-currency="id"
          > <img :src="curr.flag"> {{curr.currency_code}}</li>
        </ul>
      </div>
    </div>
  `,
  ...
  var popup_address = new Vue({
    ...
    methods: {
      submit: function(){
        ...
        // var address = Object.values(item);
        studio_app.collection_address = JSON.stringify(item);
        studio_app.addresses.push(JSON.stringify(item));
        this.visible = false;
        studio_app.show_addresses_drop = false;
      ...
      validate: function(){
        var check = {
          ...
          first_name:    'Enter your first name, please',
          last_name:    'Enter  your last name please',
        }
...
var timeout;
 var studio_app = new Vue({
    ...
    data: {
      addresses : collection_addresses_all, // array of collection addresses
      applied_coupon: '',             // applied coupons
      block_next: false,              // if true next step will not be taken
      collection_address: '',         // product's collection address if free collection option chosen
      colors: theme_colors,           //  available for product colors. Taken from product settings
      coupons: [],                    // availabe coupons' codes
      countries: all_countries_flags, // country flags object
      coupon_timer: false,            // bool - if wait spinner should be shown  in coupon field

      currency_symbol: '£',            // symbol of selected currency
      currency_index: 1,               // rate of selected currency related to the GBP
      currency_code: "GBP",            // Code of slected currency

      customize: {                    // data for customization step
        color_pref: [],
        sizes    : ['squre'],
        props    : 'none',
        position    : 'none',
      ...
      customize_step: 1,              // number of tab in customization section

      handling: {                       // shipping and return products values
        handle: 'discard',  // discard or return product
        send: 'self',       // selshipping or free collection
      },

      image_count: '-',                 // number images ordered
      may_be_coupon: '',              // value of coupon input field
      max_step: 1,                    // maximum step number within current section
      ...
      order_placed: false,            // marker if ajax add order finished
      order_total_discount: 0,        // total order discount
      prices: theme_prices,           // prices fro differen action. Created in settings> theme settings
      product_types: product_types,   // product types for product dropdown on 1st screen.  Created in settings> theme settings
      products: [                     // products to shot
        {
          title: '',
          type: '',
        },
      ...
      simple_note: '',                 // value of a simple note field
      selected_country: '',            // selected country for collection address
      show_addresses_drop: false,      // marker to show free collection address options
      step:1,                          // number of active screen of Shot Builder
      turnaround: 'regular',           // value for turnaround option

      expanded: {                      // display or hide detailed data in products summary
        products:  false,
        notes:     false,
        custom: false,
      },
      ...
      show_summary_mob: false,         // show or hide order details dropdown on mobile
    },

    watch: {
      'collection_address':function(val){
        console.log(val);
      },

      'expanded.products':function(val){
        console.log(val);
      },
      ...
      image_count: function(val){
        ...
        this.image_count = Math.max(val, 3);
      ...
      /**
      * Watchs for the notes field
      * Limits the length of this field to 140 symbols
      */
      ...
      /**
      * Watchs for the step
      * Discards order_placed parameter to trigger ajax request on return
      * to the confiemation step
      */
      step:function(step){
        if(step != 8){
    ...
    computed: {
      'addresses_formatted': function(){
         var addreses = this.addresses.map(e=>{
      ...
      _collection_address: function(){
        ...
        return this.collection_address ? Object.values(JSON.parse(this.collection_address)).join(', ') : 'No addresses saved, select to add new.';
      },
      ...
      order_total: function(){
        var product_names = (this.products.length - 1) * Math.round(this.prices.name * this.currency_index);


        var image_number = this.image_count == '-'? 0: this.image_count;

        var image_count = parseInt(image_number) * (Math.round(parseInt(this.prices.image)* this.currency_index));

        var color = Math.round(parseInt(this.prices.color)* this.currency_index)*this.customize.color_pref.length;

        var sizes = ( this.customize.sizes.length - 1 ) *  Math.round(this.prices.sizes * this.currency_index);

        var shoots = typeof(this.notes.data) =='object' ? this.notes.data.length * Math.round(this.prices.shoot* this.currency_index) : 0;

        var turnaround = this.turnaround == 'fasttrack'? Math.round(this.prices.fasttrack* this.currency_index) : 0;

        var handling = this.handling.handle == 'return'? Math.round(this.prices.handle* this.currency_index) : 0;


        // product_names = Math.round(product_names ;
        // image_count   = Math.round(image_count * this.currency_index);
        var color_sizes   = (color + sizes);
        // shoots        = Math.round(shoots*this.currency_index);
        // turnaround    = Math.round(turnaround*this.currency_index);
        // handling      = Math.round(handling*this.currency_index);
        // handling      = Math.round(handling*this.currency_index);
        ...
        total -=  Math.round(this.order_total_discount*this.currency_index);

        return {
          product_names: product_names > 0? this.currency_symbol+ product_names : 'Free' ,
          image_count: this.max_step >= 2 && image_count > 0 ? this.currency_symbol + image_count : '-' ,
          customize:   this.max_step >= 3 ? color_sizes > 0  ? this.currency_symbol + color_sizes :' Free' : '-' ,
          shoots:      this.max_step >= 4 ? shoots > 0       ? this.currency_symbol + shoots: 'Free' : '-',
          turnaround:  this.max_step >= 6 ? turnaround > 0   ? this.currency_symbol + turnaround : 'Free' : '-',
          handling:    this.max_step >= 7 ? handling > 0     ? this.currency_symbol + handling : 'Free' : '-',
    ...
    mounted: function(){
      ...
      var items = jQuery(this.$refs.scroll_image).find('.color-item');
      var width = 145* Math.ceil(items.length/2);
      jQuery(this.$refs.scroll_image).width(width);
    ...
    methods: {
      ...
      apply_address: function(addr){
        this.collection_address = addr;
        this.show_addresses_drop = false;
      },
      ...
      * fires when currency switcher component changes values
      */
      change_currency_cb:function(data){
        console.log(data);
        this.currency_symbol = data.symbol;
        this.currency_index = data.index_gpb;
        this.currency_code  = data.currency_code;

        /**
        * changes saved data for WooCommerce order
        * if currency was changed on confirm shoot screen
        */
        if(this.step == 8){
          this.order_placed = false;
          this.place_order();
      ...
      handle_image_count: function(){
        ...
        vm.image_count = value? value: '-';
      },

      handle_image_count_click: function(){
        var value = this.$refs.image_count_input.value;
        var vm = this;

        if(value == '-'){
          vm.$refs.image_count_input.value = '';
        }else{
          vm.image_count = value;
        }
      },

      parse_address:function(addr){
        return Object.values(JSON.parse(addr)).join(', ');
      },

      /**
      * Makes an Ajax call. This call sets data about order and creates a WooCommerce order
      *
      *
      */
      ...
      place_order_trigger: function(){
        if(!this.contact_number){
          this.$refs.contact_number.classList.add('error');
          return '';
        }else{
          this.$refs.contact_number.classList.remove('error')
        }
      ...
      show_popup_address: function(){
        popup_address.country = this.selected_country;
        popup_address.visible = true;
      },

      toggle_color_pref: function(color){
        if(color == 'discard'){
          this.customize.color_pref = [];
          return;
        }
      ...
      trigger_scroll: function(direction){
        // console.log(direction);

        // var sl = jQuery(this.$refs.scroll_image).scrollLeft();
        // console.log(sl);
        jQuery(this.$refs.scroll_image).scrollLeft(125)
        ...
        var margin = parseInt(jQuery(this.$refs.scroll_image).css('margin-left'));

        var delta = jQuery(this.$refs.scroll_image).width()  - jQuery(this.$refs.scroll_container).width();

        switch (direction){
          case 'right':
            margin -= 145;
            margin = Math.max(-1*delta, margin);
            break;
          case 'left':
            margin += 145;
         ...
         jQuery(this.$refs.scroll_image).css({'margin-left': margin+'px'})
      },
  ...
  gallery_app = new Vue({
    ...
    watch: {
      date_end: function(val){
        if(val && jQuery(window).width() < 992){
          var vm = this;
          Vue.nextTick(function(){
...
function init_recipe_carousel(){
  ...
  owl.siblings('.recipe-gallery__ctrl').find('.prev').click(function(){
    owl.trigger('prev.owl.carousel');
  })
  owl.siblings('.recipe-gallery__ctrl').find('.next').click(function(){
    owl.trigger('next.owl.carousel');
  })

  owl.on('changed.owl.carousel',function(e,data){
    jQuery('.recipe-gallery__counter .current').text(e.page.index + 1)
  })
}
...
function init_gallery(){
  jQuery('.shop-content__row-inner.owl-carousel').each(function(ind,el){
    var owl = jQuery(el);
    var childs = owl.find('.shop-product-item');
    var height = childs.height();
    childs.height(height);

    if(childs.length >= 2){
      owl.closest('.shop-content__row').find('.shop-content__row-prev').removeClass('visuallyhidden')

      owl.closest('.shop-content__row').find('.shop-content__row-next').removeClass('visuallyhidden')
      ...
      owl.closest('.shop-content__row').find('.shop-content__row-prev').click(function(){
        owl.trigger('prev.owl.carousel')
      })

      owl.closest('.shop-content__row').find('.shop-content__row-next').click(function(){
        owl.trigger('next.owl.carousel')
      })
    }
  })
...
function imitate_select_option(obj, text, value){
  var parent = jQuery(obj).closest('.select-imitation');
  parent.removeClass('expanded');
  parent.find('.select-imitation__view').text(text);
...
jQuery('.trigger-show').click(function(event) {
  event.preventDefault();
  jQuery('.trigger-show').removeClass('active');
  jQuery(this).addClass('active');
})
...
jQuery('.download-popup__file-imitation input').on('change', function(e){
  var names = [];
  for(var f of e.target.files){
    names.push(f.name);
  }

  jQuery(this).siblings('.text').text(names.join(', '));
})
...
jQuery('.trigger-expand').click(function(e) {
  ...
  jQuery('.resert-cells .details').each(function(index, el) {
    if (jQuery(el).data('parent') != target) {
      jQuery(el).slideUp();
    }
  });
...
/********/
jQuery('.main-menu-new > ul> li > a').click(function(e){
  e.preventDefault();
  jQuery(this).closest('li').toggleClass('expanded').siblings('li').removeClass('expanded');
  jQuery('.site-header').addClass('darken')
})

jQuery('.site-container').click(function(e){

  if(!jQuery(e.target).closest('.main-menu-new').length){
    jQuery('.site-header').removeClass('darken')
    jQuery('.main-menu-new  li').removeClass('expanded');
  }
})

jQuery('.currency-switcher').click(function() {
  jQuery(this).addClass('active')
})
...
jQuery('.currency-switcher__dropdown').hover(function() {
}, function() {
  jQuery(this).closest('.currency-switcher').removeClass('active')
});

jQuery('.currency-switcher li').click(function(e){
  e.stopPropagation();
  var html = jQuery(this).html();

  jQuery(this).closest('.currency-switcher').find('.currency-switcher__value').html(html)
  jQuery(this).addClass('active').siblings('li').removeClass('active');
   ...
   jQuery(document.body).trigger('cha_currrency',currency);
})


jQuery(document.body).on('cha_currrency', function(e,curr){
  var val = '';
  var price = parseInt(theme_settings.single_product_price);
  switch(curr){
    case 'gbp':
      val = '£'+price;
      break;
    case 'eur':
      var rate = parseFloat(currency_settings.eur);
      val = "€" +  Math.ceil(price* rate);
      break;
    case 'usd':
      var rate = parseFloat(currency_settings.usd);
      val = '$'+  Math.ceil(price* rate);;
      break;
  }

  jQuery('#price-value').text(val);
  jQuery('#price-value2').text(val);
  jQuery('#price-value3').text(val);
  jQuery('#price-value4').text(val);
})

jQuery('.faq__marker').click(function(){
  jQuery(this).closest('.faq').siblings('.faq').removeClass('active').find('.faq__text').slideUp()
  jQuery(this).closest('.faq').toggleClass('active').find('.faq__text').slideToggle()
})
...
/*
slide up and down mobile menu in footer
*/
jQuery(".footer-title").click(function(event) {
  if(jQuery(window).width()< 768){
    jQuery(this).toggleClass('active');
    // jQuery(this).siblings('.footer-menu').slideToggle();
    jQuery(this).siblings('div').find('.menu').slideToggle();
    jQuery(this).closest('.col-md-3').siblings('.col-md-3').find(".footer-title").removeClass('active');
    // jQuery(this).closest('.col-md-3').siblings('.col-md-3').find(".footer-menu").slideUp();
    jQuery(this).closest('.col-md-3').siblings('.col-md-3').find(".menu").slideUp();
  }
});

jQuery('.mobile-menu-switcher').click(function(){
  jQuery('.mobile-menu-holder').addClass('shown')
  jQuery('.user-menu').removeClass('shown')
})

jQuery('.mobile-menu-link').click(function(event) {
  // jQuery(this).closest('li').toggleClass('expanded').siblings('li').removeClass('expanded');
  jQuery(this).siblings('.main-menu-new__expand').slideToggle();
  jQuery(this).closest('li').siblings('li').find('.main-menu-new__expand').slideUp();
});

jQuery('.mobile-menu-holder__close').click(function(event) {
  jQuery('.mobile-menu-holder').removeClass('shown')
});


jQuery('.user-menu__close').click(function(event) {
  jQuery('.user-menu').removeClass('shown')
});

jQuery('.site-header-mob .gravatar, .mobile-header-style-1 .gravatar').click(function(event) {
  jQuery('.mobile-menu-holder').removeClass('shown')
  jQuery('.user-menu').addClass('shown')
})

jQuery('.product-data__tabs-list li').click(function(){
  jQuery(this).addClass('active').siblings('li').removeClass('active');
  var target = jQuery(this).data('target');
  jQuery(target).slideDown().siblings('.product-data__page').slideUp();
})


jQuery('.faq-item__title').click(function(event) {
 jQuery(this).closest('.faq-item').toggleClass('active').siblings('.faq-item').removeClass('active').find('.faq-item__text').slideUp();

  jQuery(this).siblings('.faq-item__text').slideToggle();
});
...
jQuery('.category-trigger').click(function(){
  jQuery('.mobile-categories').slideToggle();
  jQuery(this).toggleClass('active');
})


jQuery('.trigger-show').click(function(e){
  e.preventDefault();
  var target = jQuery(this).data('href');
  window.history.pushState('', '', jQuery(this).attr('href'));
  jQuery('.edit-profile-page').slideUp();
  jQuery(target).slideDown();
})

jQuery('.edit-profile-form input').on('input change', function(){
  jQuery(this).closest('.edit-profile-form').find('button').removeClass('not-active');
  jQuery(this).removeClass('error');
})
jQuery('.edit-profile-form select').on('input change', function(){
  jQuery(this).closest('.edit-profile-form').find('button').removeClass('not-active');
})

// edit profile form actions
jQuery('#my-adresses').on('submit',function(e){
  e.preventDefault();
  ...
  country.each(function(i, el) {
    if('undefined' == typeof(addresses[i])){
      addresses.push({});
    }
    addresses[i].country = el.value;
  });

  for(var id in keys){
    var items = form.find("input[name*="+keys[id]+"]");

    items.each(function(i, el) {
      addresses[i][keys[id]] = el.value;
    });
  }
  ...
  jQuery.ajax({
    ...
    type: 'POST',
    dataType: 'json',
    data: {action: 'update_collection_addresses', data: addresses},
  })
  .done(function() {
    console.log("success");
  })
...
jQuery('#my-details').on('submit',function(e){
  e.preventDefault();
  ...
  for(var i of data){
    ...
    if(!i.value){
      jQuery('input[name="'+i.name+'"]').addClass('error');
      error = true;
    }else{
      jQuery('input[name="'+i.name+'"]').removeClass('error');
    }
  ...
  jQuery.ajax({
    ...
    type: 'POST',
    dataType: 'json',
    data: {action: 'update_my_details', data: post_data},
  })
  .done(function() {
    console.log("success");
  })
...
jQuery('#my-password').on('submit',function(e){
  e.preventDefault();
  ...
  if(post_data['password_new'].length < 8){
    error = true;
    alert('Password should  be at least 8 symbols length')
    jQuery('input[name="password_new"]').addClass('error');
  }
  ...
  jQuery.ajax({
    ...
    type: 'POST',
    dataType: 'json',
    data: {action: 'update_my_password', data: post_data},
  })
  .done(function(e) {
    console.log("success");

    if(!e.old_pass_valid){
      alert('Wrong Current Password, please check it and try again')
    }
  })
  .fail(function() {
    console.log("error");
  ...
  .always(function(e) {
    button.removeClass('in-progress')
    button.addClass('not-active')
    jQuery('input[name="password_new"]').val('')
    jQuery('input[name="password_current"]').val('')
    console.log(e);
  });
})

// toggle hide show mobile dropdown
jQuery(document).on('click', '.menu-switcher', function(){
  jQuery(this).toggleClass('active');
  jQuery('.mobile-menu').toggleClass('active');
})

//  edit width of the order container for width < 992px
...
jQuery(document).ready(function(){
  if(jQuery(window).width() < 992){
    var width = parseInt(jQuery('#scroll-orders > .row').data('width'));
    jQuery('#scroll-orders > .row').width(width);
  }

})


/// show and close popup wrapper
jQuery('.carousel-product__gallery-trigger').click(function(event) {
  jQuery('.product-image-popup').addClass('shown');
  jQuery('.site-header').addClass('hidden');
  jQuery('body').addClass('limited');

  jQuery('.lazy-load-2').lazy();
});

jQuery('.product-image-popup__back, .product-image-popup__mobile-close').click(function(event) {
  jQuery('.product-image-popup').removeClass('shown');
  jQuery('.site-header').removeClass('hidden');
  jQuery('body').removeClass('limited');
});
var swipe_start = false;
var menu_touched = false;
var product_bottom_bar_touched = false;

/**
 Hide mobile nmenues on swipe
*/

jQuery('.mobile-menu-holder').on('touchstart', function(e){
  menu_touched = true;
  swipe_start = {
...
jQuery('.user-menu').on('touchstart', function(e){
  menu_touched = true;
  swipe_start = {
    x: e.originalEvent.touches[0].pageX,
    y: e.originalEvent.touches[0].pageY,
...
// jQuery('.product-mobile-bar__trigger').on('touchstart', function(e){
//   product_bottom_bar_touched = true;
//   swipe_start = {
//     x: e.originalEvent.touches[0].pageX,
//     y: e.originalEvent.touches[0].pageY,
//   };
...
// jQuery('.product-mobile-bar__header').on('touchstart', function(e){
//   product_bottom_bar_touched = true;
//   swipe_start = {
//     x: e.originalEvent.touches[0].pageX,
//     y: e.originalEvent.touches[0].pageY,
...
jQuery(document).on('touchstart', '.product-mobile-bar__trigger, .product-mobile-bar__header, .product-mobile-bar .product-data__tabs ' ,  function(e){
  product_bottom_bar_touched = true;
  swipe_start = {
    x: e.originalEvent.touches[0].pageX,
    y: e.originalEvent.touches[0].pageY,
...
jQuery('.site-container').on('touchend',function(){
  swipe_start = false;
  menu_touched = false;
  product_bottom_bar_touched = false;
})

jQuery('.site-container').on('touchmove', function(e){
  if(product_bottom_bar_touched){
    e.preventDefault();
    e.stopPropagation();
  }
  var deltaX = swipe_start.x - e.originalEvent.touches[0].pageX;
  var deltaY = swipe_start.y - e.originalEvent.touches[0].pageY;
  ...
  // close bottom bar product
  if(product_bottom_bar_touched && deltaY < -30){
    jQuery('.product-mobile-bar__inner').removeAttr('style');

    jQuery('.product-mobile-bar__content').slideUp( function(){
      product_bottom_bar_touched = false;
    });

    setTimeout(function(){
      jQuery('.product-mobile-bar__header').slideDown('fast');
    },100)
  }

  // open bottom bar product
  if(product_bottom_bar_touched && deltaY > 30){
    jQuery('.product-mobile-bar__content').slideDown(function(){
      product_bottom_bar_touched = false;
      // var height = jQuery('.product-mobile-bar').height();
      // jQuery('.product-mobile-bar__inner').height(height);
    });

    setTimeout(function(){
      jQuery('.product-mobile-bar__header').slideUp('fast');
    },100)

  }
})

jQuery('.product-mobile-bar__inner').on('touchstart ', function(e){
    if(!product_bottom_bar_touched){
      swipe_start = {
        x: e.originalEvent.touches[0].pageX,
        y: e.originalEvent.touches[0].pageY,
...
function initLazyLoad(){
  ...
  jQuery('.lazy-load').lazy({
    beforeLoad:function(el){
      if(el.hasClass('fix-height')){
        var height = el.closest('div').height();
        el.height(height);
      }
    },
...
jQuery(document).ready(function(){
  ...
  var delay = Math.floor(Math.random() * 500) + 800;

  setTimeout(function(){
    initLazyLoad();
    jQuery('.site-container').removeClass('not-ready');
  }, delay);
})


function init_testi_carousel(){
  if(jQuery('.product-testi-holder').length){
    var owl = jQuery('.product-testi-holder');
...
jQuery(document).ready(function($) {
  var left_col = jQuery('.column-left');

  if(left_col.find('.column-left__inner').height() <= jQuery(window).height() - 100){
    left_col.after('<div class="column-left-spacer"></div>').removeClass('not-fixed')
  }

  var right_col = jQuery('.right-column-fixed');

  if(right_col.find('.right-column__inner').height() <= jQuery(window).height() - 95){
    right_col.after('<div class="column-right-spacer"></div>').removeClass('not-fixed')
  }
});
...
jQuery(document).ready(function(){
  var $fixed_block = jQuery('.shoot-steps.fixed');
   $fixed_block.width($fixed_block.width());
})


jQuery(window).scroll(function(e){
  var pos = jQuery(window).scrollTop() + 60;
  var $fixed_block = jQuery('.shoot-steps.fixed');
  if(!$fixed_block.length){return;}
  var $holder = jQuery('.shoot-steps-holder');

  offset_left = offset_left< 0 ? $fixed_block.offset().left: offset_left;
  offset_top = offset_top< 0 ? $fixed_block.offset().top: offset_top;

  if(pos >= offset_top &&  pos + $fixed_block.height() < offset_top + $holder.height() ){
    $fixed_block.css({'position' : 'fixed', 'left':offset_left+'px', 'top': '60px' });
  }else{
...
jQuery(document).ready(function(){
  var $fixed_block = jQuery('.shop-side-column > .col-12');
   $fixed_block.width($fixed_block.width());
})


jQuery(window).scroll(function(e){
  var pos = jQuery(window).scrollTop() + 120;
  var $fixed_block = jQuery('.shop-side-column > .col-12');
  if(!$fixed_block.length){return;}
  if(!$fixed_block.height() > jQuery(window).height){return;}

  var $holder = jQuery('.shop-side-column');

  offset_left = offset_left< 0 ? $holder.offset().left: offset_left;
  offset_top = offset_top < 0 ? $holder.offset().top: offset_top;

  if(pos >= offset_top &&  pos + $fixed_block.height() < offset_top + $holder.height() ){
    $fixed_block.css({'position' : 'fixed', 'left':offset_left+'px', 'top': '120px', 'margin-top': 0 });
  }else if(pos + $fixed_block.height() >= offset_top + $holder.height() ){
    var martin_top = $holder.height() - $fixed_block.height();
    $fixed_block.css({'position' : 'relative', 'left':'unset', 'top': 'unset', 'margin-top': martin_top + 'px' });
  }else{
    $fixed_block.removeAttr('style')
  }
})
jQuery('.side-search input').on('input change',function(){
  var search = jQuery(this).val();
})


jQuery('.side-search form').on('submit',function(){
  var search = jQuery(this).closest('.side-search').find('input').val();

  var parent = jQuery('#all_products_block');
  var items  = parent.find('.shop-product-item');
  if (search.length < 3) {
    alert('Enter please at least 3 symbols to search');
    items.removeClass('hidden');
    return;
  }

  items.each(function(index, el) {
    var keywords = jQuery(el).data('keywords');
  ...
  var shown_elements = parent.find('.shop-product-item:not(.hidden)');

  if (shown_elements.length == 0) {
   parent.append('<p class="page-comment">No products match this search parameters</p>');
  }else{
    parent.find('.page-comment').remove();
  }

  jQuery(this).closest('.search-container').addClass('active');

  setTimeout(function(){
    var offset = parent.offset().top - 100;
