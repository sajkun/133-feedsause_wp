...
jQuery(document).ready(function(){
  var $steps_fixed = jQuery('.shoot-steps.fixed');
   $steps_fixed.width($steps_fixed.width());
...
jQuery(window).scroll(function(e){
  ...
  var $steps_fixed = jQuery('.shoot-steps.fixed');
  if(!$steps_fixed.length){return;}
  var $holder = jQuery('.shoot-steps-holder');

  offset_left = offset_left< 0 ? $steps_fixed.offset().left: offset_left;
  offset_top = offset_top< 0 ? $steps_fixed.offset().top: offset_top;

  if(pos >= offset_top &&  pos - $steps_fixed.height() < offset_top + $holder.height() ){
    $steps_fixed.css({'position' : 'fixed', 'left':offset_left+'px', 'top': '60px' });
  }else{
    $steps_fixed.removeAttr('style')
  }
})
