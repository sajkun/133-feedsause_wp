...
Vue.component('select-imitation-country', {
  ...
  template: '<div class="select-imitation" v-click-outside="discard_select"  v-bind:class="{ expanded: isExpanded, error: error}" > <select v-model="selected" v-on:change="change" v-bind:class="{ hidden: isHiddenSelect}"> <option v-for="data in options" v-bind:value="data.name">{{data.name}}</option> </select> <span class="select-imitation__view " v-on:click="expand_select"  v-bind:class="{ hidden: isHiddenImitation}"><img :src="selected_flag"> {{selected}}</span> <span class="select-imitation__arrow" onclick="imitate_select_expand(this)"></span> <div class="select-imitation__dropdown"> <ul class="select-imitation__list"> <li v-for="data in options" v-bind:class="{selected: isSelected[data], soon: !data.published}"  v-on:click="imitate_select_option(data)"> <img :src="data.flag"> <span  class="element">{{data.name}} <span class="comming-soon" v-if="!data.published">COMING SOON</span></span> </li> </ul> </div> </div>',
})
...
jQuery(document).on('click', '.summary__header', function(){
 jQuery(".summary__body").addClass('active');
});
